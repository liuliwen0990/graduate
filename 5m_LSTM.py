...
def get_train_data(filename_train, train_num_each = 100,mix_num = 5):
    num_modulations = 5
...
class LSTM(nn.Module):
    ...
    def __init__(self):
        ...
        drop1 = 0.8
        drop2 = 0.4
        self.n_features = 2 # number of parallel inputs
        ...
        self.lstm = nn.LSTM(input_size=self.n_features, hidden_size=self.n_hidden, num_layers=self.n_layers,
                            batch_first=True)
        self.linear = nn.Linear(self.n_hidden*128,5)
    ...
    def forward(self, x):
        ...
        #x = x.reshape([batch_size,1,11200])
...
def to_amp_phase(X_train,nsamples=128):
    X_train_complex = X_train[:,0,:]+1j*X_train[:,1,:]


    X_train_amp = np.abs(X_train_complex)
...
for mix in range(5,6):
    for train_num in train_num_val:
        ...
        X_train = to_amp_phase(X_train, 128)
        ...
        for l in range(num_experiment):
            ...
            row0 = ['macro-acc','macro-f1','macro-AUC','one-error','ranking_loss','train_time','epoch','SNR']
            ...
            for SNR in snr_val:
                ...
                num_test = len(x_test)
                x_test = to_amp_phase(x_test, 128)
            ...
            filename = '0412_results_LSTM_trainnum_{}_mix_{}.xls'.format(train_num,mix)
            workbook.save(filename)
            sys.exit()
